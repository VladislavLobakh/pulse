...
| Parallel research | Generic fan-out engine (`patterns/parallel.py`): run source runners concurrently, isolate failures, dedup merged items by normalized URL (**Implemented**, HN and non-ReAct ArXiv runners wired via `agents/hn.py` and `agents/arxiv.py`; remaining sources Planned) | `SourceOutput`, `SourceRunResult`, `ParallelRunResult` | [`parallel-collect.mmd`](architecture/flows/parallel-collect.mmd) |
