...
| Parallel research | Generic fan-out engine (`patterns/parallel.py`): run source runners concurrently, isolate failures, dedup merged items by normalized URL (**Implemented**, HN plus non-ReAct ArXiv and YouTube runners wired via `agents/hn.py`, `agents/arxiv.py` and `agents/youtube.py`; remaining sources Planned) | `SourceOutput`, `SourceRunResult`, `ParallelRunResult` | [`parallel-collect.mmd`](architecture/flows/parallel-collect.mmd) |
