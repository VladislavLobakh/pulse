...
from pulse.agents.hn import MIN_ARTICLES, run_hn_react
from pulse.display import print_articles, print_trace, warn_if_below_minimum


def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="PULSE — collect articles from Hacker News.")
    parser.add_argument("query", help="Search query.")
    args = parser.parse_args(argv if argv is not None else sys.argv[1:])

    print("PULSE — collecting articles from Hacker News...")
